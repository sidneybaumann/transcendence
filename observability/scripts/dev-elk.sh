...
KIBANA_IMPORT_URL="http://localhost:5601/api/saved_objects/_import?overwrite=true"
...
attempt=1
until curl -fsS -u "$ES_USER:$ES_PASSWORD" "$ES_URL/_cluster/health" >/dev/null 2>&1; do
...
curl -fs -u "elastic:$ELASTIC_PASSWORD" \
  -H "Content-Type: application/json" \
  -X PUT "http://localhost:9200/_ilm/policy/ft-transcendence-logs-policy" \
  -d '{
    "policy": {
      "phases": {
        "hot": {
...
curl -fs -u "elastic:$ELASTIC_PASSWORD" \
  -H "Content-Type: application/json" \
  -X PUT "http://localhost:9200/_index_template/ft-transcendence-template" \
  -d '{
...
attempt=1
until curl -fs "$KIBANA_URL" >/dev/null; do
  if [ "$attempt" -ge "$MAX_ATTEMPTS" ]; then
...
echo "Waiting for Kibana saved objects API to be ready..."

attempt=1
until curl -fsS -u "$ES_USER:$ES_PASSWORD" \
  -H "kbn-xsrf: true" \
  "$KIBANA_URL/api/status" >/dev/null 2>&1; do
  if [ "$attempt" -ge "$MAX_ATTEMPTS" ]; then
    echo "Error: Kibana API is not ready after $((MAX_ATTEMPTS * SLEEP_SECONDS)) seconds."
    exit 1
  fi

  echo "Kibana API not ready yet... ($attempt/$MAX_ATTEMPTS)"
  attempt=$((attempt + 1))
  sleep "$SLEEP_SECONDS"
done
...
if [ -f "$KIBANA_SAVED_OBJECTS_FILE" ]; then
  ...
  http_code="$(curl -sS -u "elastic:$ELASTIC_PASSWORD" -o "$import_response" -w "%{http_code}" \
...
attempt=1
until curl -fs "$LOGSTASH_URL" >/dev/null; do
  if [ "$attempt" -ge "$MAX_ATTEMPTS" ]; then
